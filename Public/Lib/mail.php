 ...
 * @example
 * $mail = new MySendMail();
 * $mail->setServer("smtp@126.com", "XXXXX@126.com", "passwd"); //设置smtp服务器，普通连接方式
 * $mail->setServer("smtp.gmail.com", "XXXXX@gmail.com", "passwd", 465, true); //设置smtp服务器，到服务器的SSL连接
 * $mail->setFrom("from_email"，"email_title"); //设置发件人
 * $mail->setReceiver("to_email"); //设置收件人，多个收件人，调用多次
 * $mail->setCc("to_email"); //设置抄送，多个抄送，调用多次
 * $mail->setBcc("to_email"); //设置秘密抄送，多个秘密抄送，调用多次
 * $mail->addAttachment("add_files"); //添加附件，多个附件，调用多次
...
class MySendMail {
	 ...
	 * @return boolean
	 */
	public function setFrom($from, $title) {
		$this->_from = $from;
		$this->_title = $title;
		return true;
	}
	...
	protected function getCommand() {
		...
		$header = "FROM:" . $this->_title . " <" . $this->_from . ">\r\n";
	...
	protected function socketSecurity() {
		...
		//**************		解决：在阿里云虚拟主机配置邮件发信失败  Start*******************
		/**
		 * @authors Zhu Peng (rieeqms@outlook.com)
		 * @date    2017-05-28 16:04:31
		https://help.aliyun.com/knowledge_detail/36419.html
		把：
		resource stream_socket_client ($host, $errno, $errstr, $port)
		替换为：
		fsockopen($host, $port, $errno, $errstr);
		 */
		if (empty($this->_socket = stream_socket_client($remoteAddr, $errno, $errstr, 30))) {
			$this->_socket = fsockopen($remoteAddr, 30, $errno, $errstr);
		}
		//**************		解决：在阿里云虚拟主机配置邮件发信失败   END*******************
		if (!$this->_socket) {
...
function send_mail($To_em, $subject, $contenttext) {
	try {
		include $_SERVER['DOCUMENT_ROOT']."/config.php";

		$mail = new MySendMail();
		// $mail->setServer("smtp@qq.com", "185417081@qq.com", "XXXXX"); //设置smtp服务器，普通连接方式
		$mail->setServer("$SMTP_Sersers", "$Email_Account", "$Email_Passwd", 465, true); //设置smtp服务器，到服务器的SSL连接
		// $mail->setServer("smtp.myeit.site", "admin@myeit.site", "Zhupeng_ad"); //设置smtp服务器，到服务器的SSL连接
		$mail->setFrom("$Email_Account", "云南师范大学失物招领系统"); //设置发件人
		$mail->setReceiver("$To_em"); //设置收件人，多个收件人，调用多次
		// $mail->setCc("XXXX"); //设置抄送，多个抄送，调用多次
		// $mail->setBcc("XXXXX"); //设置秘密抄送，多个秘密抄送，调用多次
		// $mail->addAttachment("XXXX"); //添加附件，多个附件，调用多次
		$mail->setMailInfo("$subject", "$contenttext"); //设置邮件主题、内容
		$mail->sendMail(); //发送
		return true;
	} catch (Exception $e) {
		echo $e->getMessage();
...
// send_mail("185417081@qq.com", "fff", "DD");
// *******************      TEST    ************
// $mail = new MySendMail();
// // $mail->setServer("smtp@qq.com", "185417081@qq.com", "XXXXX"); //设置smtp服务器，普通连接方式
// $mail->setServer("smtp.qq.com", "185417081@qq.com", "ueqnehwmauuvcbcc", 465, true); //设置smtp服务器，到服务器的SSL连接
// // $mail->setServer("smtp.myeit.site", "admin@myeit.site", "Zhupeng_ad"); //设置smtp服务器，到服务器的SSL连接
// $mail->setFrom("185417081@qq.com", "云南师范大学失物招领系统"); //设置发件人
// $mail->setReceiver("185417081@qq.com"); //设置收件人，多个收件人，调用多次
// // $mail->setCc("XXXX"); //设置抄送，多个抄送，调用多次
// // $mail->setBcc("XXXXX"); //设置秘密抄送，多个秘密抄送，调用多次
// // $mail->addAttachment("XXXX"); //添加附件，多个附件，调用多次
// $mail->setMailInfo("找回密码0", "<b>test</b>"); //设置邮件主题、内容
// $mail->sendMail(); //发送